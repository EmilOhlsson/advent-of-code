...
impl Intmachine {
    ...
    pub fn load(input: &str, buffer: i64) -> Intmachine {
        ...
        program.resize(1024, program_length as i64 + buffer);
        Intmachine {
            output: Vec::new(),
            program,
            trace: Vec::new(),
    ...
    pub fn run_to_event(&mut self, input: Option<i64>) -> IOState<i64> {
        loop {
    ...
    fn run_instruction(&mut self, input: Option<i64>) -> Option<IOState<i64>> {
        if self.debug {
